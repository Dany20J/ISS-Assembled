...
import { CSR, Certificate, SignatureData } from "./types";
...
function moveElement(fromArray: any[], toArray: any[], index: number) {
...
app.post("/admin/approve-csr", (req, res) => {
  ...
  const pendingRequestIndex = pendingCSRs.findIndex(
    (request) => request.phoneNumber === body.phoneNumber
  );
...
app.post("/post-csr", (req, res) => {
  ...
  const body = req.body as CSR;

  if (
    pendingCSRs.find((csr) => csr.phoneNumber === body.phoneNumber) ||
...
app.post("/get-cert", (req, res) => {
  ...
  const approvedRequest = approvedCSRs.find(
    (request) => request.phoneNumber === body.phoneNumber
  );
